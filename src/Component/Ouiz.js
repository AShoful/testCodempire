...
function Quiz({ quiz, handleSetAnswer, answers, isDisabled }) {
  ...
  const countNoQuestions = quiz.questions.length - arrAnswers.filter((item) => item).length;
  ...
  return (
    <Box className={classes.root}>
      ...
      {quiz.questions.map((item, i) => renderUIComponents({ item, i, handleSetAnswer, arrAnswers, isDisabled }))}
      <ContainedButtons button={button} linkName="Rezult" path="/rezults" />
      <Prompt
        when={!!countNoQuestions}
        message={() => `Вы не оветили, или не сохранили ответы на 
